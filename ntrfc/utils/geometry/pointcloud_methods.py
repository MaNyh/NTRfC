...
def extractSidePolys(ind_hk, ind_vk, sortedPoly):
    ...
    if ind_vk > ind_hk:
        side_one_idx = indices[ind_hk:ind_vk]
        side_two_idx = np.concatenate((indices[:ind_hk][::-1], indices[ind_vk-1:][::-1]))
    elif ind_vk < ind_hk:
        side_one_idx = indices[ind_vk:ind_hk]
        side_two_idx = np.concatenate((indices[:ind_vk][::-1], indices[ind_hk-1:][::-1]))
